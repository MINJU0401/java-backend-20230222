...
//		if문의 반복 형태
//		반복 실행할 코드블럭이 종료되면 조건을 다시 검사하고
//		true면 반복, false면 탈출
//		사용방법 : while (조건) { 실행할 코드 }
		int number = 0;
		while (true) {
//			continue : continue문을 만나면 코드블럭내에서 아래에 오는 모든 코드를 무시하고 다시 조건을 검사 
//			일반적으로 continue문은 조건문과 함께 쓰임
//			! 조건문의 코드 블럭 내에 해당 변수를 변경시켜주는 코드가 존재해야 함 (while문에서만)
			if ((number % 2) == 0) { number ++; 
			continue;
			}
			System.out.println(number);
//			while문에서는 조건에 사용되는 변수가 필수로 실행코드 내에서 변경이 이루어져야 함
			number++;
//			break : 반복문의 코드블럭을 강제로 종료시킬 때 사용
