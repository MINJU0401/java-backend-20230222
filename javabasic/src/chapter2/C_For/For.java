...
//	for 반복문
//	규칙이 있는 작업을 반복하는 제어문
//	일반적으로 반복 횟수가 정해져 있을 때 사용하는 반복문(반복횟수가 명확하지 않을때는 while문을 쓴다)
...
//		실행 순서 : 1)변수선언 및 초기화 2)조건 3)실행할코드 4)증감식
		
		for (int index = 0; index < 5; index++) {
			intArray5[index] = index + 1;
//			System.out.println("intArray5[" + index + "] = " + (index + 1));
		}
		
//		for문의 경우 코드 블럭에 오는 실행할 코드가 한 구문이면 {}를 제거할 수 있음
		for (int index = 0; index < 5; index++)
			intArray5[index] = index + 1;
		
//		foreach 반복문
//		배열의 요소 값을 직접 읽고자 할 때 배열을 반복하면서 요소를 하나씩 복사해서 사용할 수 있음
//		사용방법 : for (배열요소의 데이터타입 변수명 : 배열) { 실행할 코드 }
