...
//	예외 : 정상적인 입력에서는 정상 처리가 되지만 비정상적인 입력에서는 처리가 되지 않는 문제 (null 값 참조, 실행중 데이터 베이스 연결 실패)

public class ExceptionHandling {
	
	//	throw 예외클래스 : 해당 예외를 호출부로 책임을 전가
...
//		System.out.println(numbers[3]);
		...
		//	가장 좋은 것 (예외가 발생하지 않도록 하는 것 < 최우선임)
		
		//	try - catch 문으로 예외 잡기 (매우 중요)
		//	try 구문에 예외가 발생할 수 있는 문장을 작성
		//	catch 구문에 해당 예외 상황에 대한 대처 구문을 작성
		
		//	사용 방법
		//	try { 예외가 발생할 수 있는 문장 }
		//	catch ( 예외 타입 예외참조변수 ) { 예외가 발생했을 때 처리할 문장 }
		
		Scanner scanner = null;
		
		try { 
			int number = scanner.nextInt();
		} catch (Exception exception) {   // catch 예외 타입 예외참조변수 자리에는 정확한 참조변수명을 입력해주면 좋지만, 
										  // 모를 경우 모든 예외형태의 참조변수인 Exception으로 작성하여도 처리에 무리는 없음
